...
.pull-right[
```{r view-follow, echo = FALSE, gganimate = list(nframes = 20), cache = TRUE}
penguins %>%
  drop_na() %>%
  ggplot(aes(x = flipper_length_mm, y = body_mass_g)) +
  geom_point() +
...
.pull-left[
```{r shadow-wake, echo = FALSE, gganimate = list(nframes = 20)}
penguins %>%
  drop_na() %>%
  ggplot(aes(x = flipper_length_mm, y = body_mass_g)) +
  geom_point(size = 2) +
...
.pull-right[
```{r shadow-mark, echo = FALSE, gganimate = list(nframes = 20), cache = TRUE}
...
.pull-right[
```{r ref.label="dino-5", echo=FALSE, cache=TRUE, gganimate = list(nframes = 30), cache = TRUE}
```
]
---
